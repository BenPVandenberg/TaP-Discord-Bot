...
const PLAY_PATH = "./data/play.json";
        ...
        for (const game in game_data[user]["gameLogs"]) {
            ...
            if (!existing_log)
                all_games.push({
                    Title: title,
                    GameID:
                        game.trim() === title ? null : parseInt(game.trim()),
                });
            else if (existing_log.GameID === null)
                existing_log.GameID = parseInt(game.trim());
        }
    }
...
function fetch_users() {
    ...
    for (const user in game_data) {
        ...
        all_users.push({
            ...
            Discriminator: parseInt(identity["discriminator"]),
        });
    }
...
function fetch_voice_sessions() {
    ...
    for (const user in voice_data) {
        for (const session of voice_data[user]["sessions"]) {
            if (!all_logs.find((e) => e.SessionID === session["sessionID"]))
                all_logs.push({
                    UserID: parseInt(user),
    ...
    for (const user in voice_data) {
        for (const session of voice_data[user]["sessions"]) {
            for (const event of session["events"]) {
                const existing_log = all_channels.find(
                    (e) => e.ChannelID === parseInt(event["channelID"]),
                );
        ...
        for (const session of voice_data[user]["sessions"]) {
            ...
            for (const event of session["events"]) {
                if (event.type === "join") {
                    ...
                    event.type === "leave" &&
                    prev_join !== null &&
                    event.channelID === prev_join.channelID
                ) {
                    all_logs.push({
                        SessionID: session["sessionID"],
                        ChannelID: parseInt(event.channelID),
                        start: prev_join["timestamp"],
                        end: event["timestamp"],
                    });
                    prev_join = null;
                }
...
function fetch_play_logs() {
    const play_data = JSON.parse(fs.readFileSync(PLAY_PATH));
    const rtn_data = [];

    for (const sound in play_data) {
        for (let i = 0; i < play_data[sound]; i++) {
            rtn_data.push({ SoundName: sound, Timestamp: null });
        }
    }

    fs.writeFileSync("./play_logs.json", JSON.stringify(rtn_data));
}
