  ...
  try {
    await PoolClass.makeSQLQuery(
      'INSERT IGNORE INTO User (UserID, DisplayName, UserName, Discriminator) ' +
        'VALUES (?, ?, ?, ?);',
      [user.id, user.displayName, user.user.username, user.user.discriminator]
    );
    await PoolClass.makeSQLQuery(
      'UPDATE User SET ' +
        'DisplayName = ?, ' +
        ...
        'Discriminator = ? ' +
        'WHERE UserID = ?',
      [user.displayName, user.user.username, user.user.discriminator, user.id]
    );
  } catch (e) {
...
export async function dbMakeVoiceLog(
  ...
  try {
    await verifyUser(user);
    await PoolClass.makeSQLQuery(
      'INSERT IGNORE INTO VoiceSession (SessionID, UserID) ' + 'VALUES (?, ?);',
      [sessionID, user.id]
    );
    await PoolClass.makeSQLQuery(
