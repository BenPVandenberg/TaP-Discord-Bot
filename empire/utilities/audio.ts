...
export async function sendSoundList(
  ...
  const nonHiddenSounds = allSounds.filter((val) => hiddenSounds.includes(val));
  ...
  for (const soundIndex in nonHiddenSounds) {
    const curSound = nonHiddenSounds[soundIndex].slice(0, -4);
    currentfield = Math.floor(
      Number(soundIndex) / (nonHiddenSounds.length / 3)
    );
