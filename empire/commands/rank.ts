...
module.exports = {
  ...
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((subCommand) =>
      subCommand.setName('list').setDescription('Lists all the available ranks')
    )
    .addSubcommand((subCommand) =>
      subCommand
        .setName('add')
        ...
        .addUserOption((option) =>
          option.setName('user').setDescription('The user to add the rank to')
        )
    )
    .addSubcommand((subCommand) =>
      subCommand
...
async function editRank(
  ...
  if (operation === 'add') {
    ...
    return await interaction.reply({
      content: `Removed ${rankToAdd.name} from ${memberToEdit.toString()}`,
    });
  }
}
